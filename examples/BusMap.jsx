...
function BusMap() {
  ...
  useEffect(() => {
    ...
    fetch(`https://sg-bus-api.vercel.app/api/bus-stops/geojson?bbox=${bbox}&limit=200`)
      .then(res => res.json())
      .then(data => setBusStops(data.features))
  ...
  const loadArrivals = async (stopCode) => {
    try {
      const response = await fetch(`https://sg-bus-api.vercel.app/api/realtime/arrivals?busStopCode=${stopCode}`);
      const data = await response.json();
      setArrivals(prev => ({ ...prev, [stopCode]: data.arrivals }));
    } catch (error) {
    ...
    <MapContainer center={[1.3521, 103.8198]} zoom={11} style={{ height: '100vh' }}>
      ...
      {busStops.map(stop => (
        <Marker 
          ...
          eventHandlers={{
            click: () => loadArrivals(stop.properties.number)
          }}
        >
          <Popup>
            <div>
              <h3>{stop.properties.name}</h3>
              <p>Stop: {stop.properties.number}</p>
              <p>Road: {stop.properties.road}</p>
              <p>Services: {stop.properties.services.join(', ')}</p>
              
              {arrivals[stop.properties.number] && (
                <div>
                  <h4>Next Arrivals:</h4>
                  {arrivals[stop.properties.number].map(arrival => (
